CLASS zcl_data_file_upload_5364 DEFINITION
PUBLIC
CREATE PUBLIC .
  PUBLIC SECTION.
...
CLASS zcl_data_file_upload_5364 IMPLEMENTATION.
    ...
    CASE request->get_method( ).
      WHEN CONV string( if_web_http_client=>get ).
        ...
        ELSE.
          ...
          xco_cp_abap_sql=>constraint->contains_pattern( to_upper(
          sap_table_request ) && '%' ) ).
...
* extract the filename and fileext from this metadata as well as the raw file data.
        SPLIT request->get_text( ) AT cl_abap_char_utilities=>cr_lf INTO TABLE
        DATA(content).
